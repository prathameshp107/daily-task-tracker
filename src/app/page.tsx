      ...
      <main className="flex-1 flex flex-col p-4 md:p-6 max-w-7xl w-full mx-auto">
        <div className="flex items-center justify-between mb-6">
          ...
          <Dialog open={isModalOpen} onOpenChange={setIsModalOpen}>
            ...
            <DialogContent className="max-w-2xl w-full max-h-[90vh] overflow-y-auto p-6">
              {/* Close button */}
              <button
                onClick={() => { setIsModalOpen(false); setCurrentTask(null); }}
              ...
              <div className="mb-2">
                ...
                <p className="text-muted-foreground text-sm mt-1">
                  {currentTask ? 'Update the details of your task below.' : 'Fill in the details to add a new task to your tracker.'}
                </p>
              </div>
              <hr className="my-4" />
              <TaskForm 
                  ...
                  if (currentTask) {
                    setTasks(tasks.map(t => 
                      t.taskId === currentTask.taskId 
                        ? { ...taskData, taskId: currentTask.taskId, completed: t.completed } 
                        : t
                    ));
                  } else {
                    setTasks([...tasks, { 
        ...
        <div className="mt-6 text-center text-sm text-muted-foreground">
          {tasks.length === 0 ? (
            ...
            <p>{tasks.length} {tasks.length === 1 ? 'task' : 'tasks'} in total</p>
          )}
        </div>
      </main>
